...
pair[i]    = round(W(i+1)/n) − round(Wi/n)          # 齿距的相邻差分，Σpair === totalUnits
top[i]     = round((pair[i] + d) / 2)               # d = round(2×斜移 / 0.1)
root[i]    = pair[i] − top[i]                       # 互补：齿顶＋齿根 === 齿距（逐格相等）
边距       = root[末齿] / 2                          # 左右同一值；末齿齿根为奇数格时为 x.x5
```

- 布局：`左边距｜齿顶1｜齿根1(=槽1)｜齿顶2｜…｜齿顶n｜右边距`，前 n−1 个齿根即齿间槽，末齿齿根拆成两个边距 → Σ齿顶＋Σ齿根 严格等于板宽，末齿齿右端＋右边距 = 板宽
- 半隐燕尾：齿深 = 0.75 × 板厚；穿透燕尾齿深 = 板厚；齿顶宽在槽底（最深面）

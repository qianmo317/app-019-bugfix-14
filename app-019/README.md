...
| 燕尾齿宽分配 | 0.05mm 闭合格子整数分配；逐齿「齿顶＋齿根＝齿距」，左右半齿边距相等，Σ齿顶宽 + Σ齿根宽 = 板宽 |
...
直接对每齿独立四舍五入会累积误差，本实现先在闭合格子上求整数关系，再生成坐标：

```text
G          = 0.05mm（偶数齿遇到奇数总格数时回退 0.025mm）
T          = round(板宽 / G)
d          = 最接近 2×斜移/G 且满足奇偶约束的整数格
q          = (T − n×d) / 2                  # n 个齿根宽合计（整数格）
root[i]    = 整数格分配，两端齿根等宽且可平分
top[i]     = root[i] + d
pitch[i]   = top[i] + root[i]
leftMargin = root[0] / 2，rightMargin = root[n-1] / 2
```

- 顺序展开：左边距 + 齿1顶 + 齿1根槽 + 齿2顶 + … + 齿n顶 + 右边距；末齿齿根由左右两个半齿边距合计承担
- 因此 `Σ(top[i] + root[i]) = 板宽`、`top[i] + root[i] = pitch[i]`，末齿右缘 + 右边距严格到板边
- 半隐燕尾：齿深 = 0.75 × 板厚；齿顶宽在展示面，背面槽口按斜移收窄
...
| 层级 | 数量 | 覆盖点 |
| --- | --- | --- |
| 单元（vitest） | 44 | 随机 200 组燕尾严格闭合（mulberry32 固定种子）、逐齿齿距/等宽边距核对、直榫手工核算、三视图一致性、边界警告、锯切线数量、导出导入往返全等、性能 |
...
- [x] 齿宽分配 200 组随机严格闭合（闭合误差 0），逐齿齿顶＋齿根＝齿距，左右边距相等，低于最小值给警告
